...
L.Control.GeoSearch = L.Control.extend({
    ...
    resetLink: function (extraClass) {
        ...
        link.className =
            "leaflet-bar-part leaflet-bar-part-single" + " " + extraClass;
    },
    ...
    onAdd: function (map) {
        ...
        input.placeholder = this._config.searchLabel;

        // create the error message div
        var message = L.DomUtil.create(
            "div",
            "leaflet-bar message displayNone",
            this._container
        );
    ...
    geosearch: function (qry) {
        try {
            ...
            if (typeof provider.GetLocations == "function") {
                var results = provider.GetLocations(
                    qry,
                    function (results) {
    ...
    _processResults: function (results) {
        if (results.length === 0) throw this._config.notFoundMessage;
    ...
    _showLocation: function (location) {
        if (this._config.showMarker) {
            if (typeof this._positionMarker === "undefined") {
                this._positionMarker = L.marker([location.Y, location.X]).addTo(
                    this._map
                );
                this._positionMarker.on('click', () => {
                    this._map.removeLayer(this._positionMarker);
                    this._positionMarker = undefined;
                });
            } else this._positionMarker.setLatLng([location.Y, location.X]);
        }
